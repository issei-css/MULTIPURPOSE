...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($stmt->execute()) {
        $message = "Savings interest rule added.";
    } else {
...
// Fetch all rules
$rules = $conn->query("SELECT * FROM savings_interest_rates ORDER BY min_balance ASC");
?>
...
<head>
    ...
    <title>Savings Interest Rules</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="content">
        <!-- Top Bar -->
        <div class="top-bar">
            <div class="welcome">Savings Interest Rules</div>
        ...
        <!-- Add Interest Rule Form -->
        <form method="POST">
            <h3 style="margin-top: 0;">Add New Interest Rule</h3>
            
            <label for="min_balance">Min Balance (₱)</label>
            <input type="number" name="min_balance" id="min_balance" step="0.01" required>
            
            <label for="max_balance">Max Balance (₱)</label>
            <input type="number" name="max_balance" id="max_balance" step="0.01" required>
        ...
        <!-- Interest Rules Table -->
        <h3>Defined Interest Rules</h3>
        <table>
            <thead>
                <tr>
                    <th>#</th>
            ...
            <tbody>
                <?php $i = 1; while($r = $rules->fetch_assoc()): ?>
                    <tr>
                        <td><?php echo $i++; ?></td>
                        <td>₱<?php echo number_format($r['min_balance'], 2); ?></td>
                        <td>₱<?php echo number_format($r['max_balance'], 2); ?></td>
